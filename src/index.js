import "./../assets/scss/app.scss";

import "./../assets/img/icons/logo.svg";
import "./../assets/img/icons/home-icon.svg";
import "./../assets/img/icons/user-icon.svg";
import "./../assets/img/icons/calendar-icon.svg";
import "./../assets/img/icons/phone-icon.svg";
import "./../assets/img/icons/account-icon.svg";
import "./../assets/img/icons/filter-icon.svg";
import "./../assets/img/icons/search-icon.svg";
import "./../assets/img/icons/arrow-down.svg";
import "./../assets/img/icons/download-arrow-down.svg";
import "./../assets/img/icons/check-mark.svg";
import "./../assets/img/icons/doctor-icon.svg";
import "./../assets/img/icons/notes-icon.svg";
import "./../assets/img/icons/clock-icon.svg";
import "./../assets/img/icons/play-icon.svg";
import "./../assets/img/icons/heart-icon.svg";
import "./../assets/img/icons/calendar-colored-icon.svg";
import "./../assets/img/icons/arrow-left-icon.svg";
import "./../assets/img/icons/clip-icon.svg";

import "./../assets/js/sidebarToggle";
import "./../assets/js/selector";
